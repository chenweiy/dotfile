...
static int _mk_format_spiffs(spiffs *fs)
{
    SPIFFS_unmount(fs);
...
void *mk_spiffs_mount(int phys_size,
                      int phys_addr,
...
#define SPIFFS_CACHE_BUF_SIZE (phys_size) //(log_page_size+32)*128)
    ...
    res = SPIFFS_mount(pfs, &cfg, d->spiffs_work_buf,
                        d->spiffs_fds, SPIFFS_FDS_SIZE,
                        d->spiffs_cache_buf, SPIFFS_CACHE_BUF_SIZE, 0);

    if (res != SPIFFS_OK) {
        printf("Initializing SPIFFS ...\n");
        if (_mk_format_spiffs(pfs)) {
            res = SPIFFS_mount(pfs, &cfg, d->spiffs_work_buf,
                    d->spiffs_fds, SPIFFS_FDS_SIZE,
                    d->spiffs_cache_buf, SPIFFS_CACHE_BUF_SIZE, 0);
        } else {
...
int mk_spiffs_umount(spiffs *fs)
{
    SPIFFS_clearerr(fs);
    SPIFFS_unmount(fs);
