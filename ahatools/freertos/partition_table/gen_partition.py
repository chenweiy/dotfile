#!/usr/bin/env python
#
# VATICS-RTOS partition table generation tool
#
...
class PartitionTable(list):
    ...
    def to_csv(self, simple_formatting=False):
        rows = [ "# VATICS-RTOS Partition Table",
...
class PartitionDefinition(object):
    ...
    # Keep this map in sync with vtcs_partition_subtype_t enum in vtcs_partition.h 
    SUBTYPES = {
        APP_TYPE : {
            "factory" : 0x00,
            "test" : 0x20,
    ...
    # add subtypes for the 5 spiffs slot values ("spiffs_XXX, etc.")
    for spiffs_slot in range(5):
        SUBTYPES[TYPES["data"]]["spiffs_%d" % spiffs_slot] = 0x10 + spiffs_slot
...
def main():
    global quiet
    parser = argparse.ArgumentParser(description='VATICS-RTOS partition table utility')
